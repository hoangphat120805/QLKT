...
import NCKHProposalForm from '@/components/proposals/NCKHProposalForm';
...
export default function CreateProposalPage() {
  ...
  const [nckhData, setNckhData] = useState<any[]>([]); // State lưu data NCKH
  ...
  const handleSubmit = async () => {
    ...
    if (proposalType === 'NCKH') {
      if (nckhData.length === 0) {
        antMessage.error('Vui lòng thêm ít nhất một thành tích ĐTKH/SKKH!');
        return;
      }
    }
    // Validation cho các loại khác
    else if (!fileExcel.length) {
    ...
    try {
      ...
      // Đối với NCKH, gửi data JSON thay vì Excel
      if (proposalType === 'NCKH') {
        // Tạo fake Excel file với data JSON
        const jsonBlob = new Blob([JSON.stringify(nckhData, null, 2)], {
          type: 'application/json',
        });
        const fakeExcel = new File([jsonBlob], 'nckh_data.json', { type: 'application/json' });
        formData.append('file_excel', fakeExcel);
        formData.append('nckh_data', JSON.stringify(nckhData));
      } else {
        ...
        <>
          ...
          {proposalType === 'NCKH' && (
            <Card
              title={
                <Space>
                  <ExperimentOutlined style={{ fontSize: 20, color: '#52c41a' }} />
                  <span style={{ fontSize: 18 }}>Thông tin đề xuất - ĐTKH/SKKH</span>
                </Space>
              }
              style={{ marginBottom: 24 }}
              headStyle={{ backgroundColor: '#f6ffed', borderBottom: '2px solid #b7eb8f' }}
            >
              <NCKHProposalForm onDataChange={setNckhData} initialData={nckhData} />
            </Card>
          )}
          ...
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              {/* Summary info cho NCKH */}
              {proposalType === 'NCKH' && (
                <div style={{ color: '#52c41a', fontWeight: 500 }}>
                  <ExperimentOutlined /> Đã thêm: <strong>{nckhData.length}</strong> thành tích
                  ĐTKH/SKKH
                </div>
              )}
              <div style={{ flex: 1 }} />
              <Space size="middle">
                ...
                <Button
                  ...
                  disabled={proposalType === 'NCKH' && nckhData.length === 0}
                >
                  Gửi đề xuất
                </Button>
              </Space>
