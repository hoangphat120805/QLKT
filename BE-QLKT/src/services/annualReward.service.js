      ...
      if (danh_hieu && !validDanhHieu.includes(danh_hieu)) {
        throw new Error(
          "Danh hiệu không hợp lệ. Danh hiệu hợp lệ: " +
            validDanhHieu.join(", ") + " (hoặc null = không đạt)"
        );
      }
    ...
    try {
      ...
      const updatedReward = await prisma.danhHieuHangNam.update({
        ...
        data: {
          ...
          so_quyet_dinh_bkbqp:
            so_quyet_dinh_bkbqp !== undefined
              ? so_quyet_dinh_bkbqp
              : reward.so_quyet_dinh_bkbqp,
          nhan_cstdtq:
            nhan_cstdtq !== undefined ? nhan_cstdtq : reward.nhan_cstdtq,
          so_quyet_dinh_cstdtq:
            so_quyet_dinh_cstdtq !== undefined
              ? so_quyet_dinh_cstdtq
  ...
  async importFromExcelBuffer(buffer) {
    try {
      ...
      for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        const danh_hieu_raw = String(
          row.getCell(headerMap["danh_hieu"]).value || ""
        ).trim();
      ...
      for (const personnelId of personnel_ids) {
        try {
          ...
          try {
            ...
            console.error(`⚠️ Failed to auto-recalculate profile for personnel ${personnelId}:`, recalcError.message);
          }
        } catch (error) {
          errors.push({
            personnelId,
